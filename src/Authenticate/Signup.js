...
function Signup({ onSubmit, error, toLogIn, toSignUp, alreadyHaveAcc }) {
  ...
  const handleSignupSubmit = async (e) => {
    ...
    if (!validUsername.test(formData.username)) {
      ...
      setSignupError(
        "Password length >6 Letter(begin UpperCase), Number, Special characters"
      );
      return;
    }
    ...
    <div className="flex justify-center px-10 sm:px-0 items-center h-screen">
      ...
      >
        <div className="flex cursor-pointer bg-stone-100 rounded-xl w-32">
          <span
            className={`px-1 flex justify-center w-1/2 rounded-xl ${
              alreadyHaveAcc ? "bg-[#2de336] text-white" : "text-black"
            }`}
            onClick={toLogIn}
          >
            Login
          ...
          <span
            ...
            className={`${
              !alreadyHaveAcc ? "bg-[#2de336] text-white" : "text-black"
            } rounded-xl flex justify-center w-1/2`}
          >
            Signup
          </span>
        </div>
        ...
        <div className="flex w-3/4 bg-white border-b border-stone-400">
          <input
            ...
            name="repassword"
            onChange={handleChange}
            className="w-11/12 text-sm border outline-none"
          ></input>
          <span
        ...
        {isLoading ? (
          <div className="text-center">
            Loading...{" "}
            {/* You can replace this with your loading icon or component */}
          </div>
        ) : (
          <input
            type="submit"
